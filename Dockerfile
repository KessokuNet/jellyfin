...
RUN curl -fsSL https://deno.land/install.sh | DENO_INSTALL=/usr sh -s -- --yes
...
RUN unzip /var/ffmpeg-over-ip-client.zip -d /tmp/ && \
    ...
    mkdir -p /usr/lib/ffmpegip && \
    ln -s /usr/local/bin/ffmpeg-over-ip-client /usr/lib/ffmpegip/ffmpeg && \
    ln -s /usr/local/bin/ffmpeg-over-ip-client /usr/lib/ffmpegip/ffprobe && \
    rm -rf /var/ffmpeg-over-ip-client.zip /tmp/ffmpeg-over-ip-client

# COPY ./rffmpeg.yml /etc/rffmpeg/rffmpeg.yml
ENV JELLYFIN_FFMPEG=/usr/lib/ffmpegip/ffmpeg
